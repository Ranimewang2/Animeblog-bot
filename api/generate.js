...
export default async function handler(req, res) {
  ...
  try {
    ...
    // Pick randomly from full pool — spread across all types
    const randomIndex = Math.floor(Math.random() * topicPool.length);
    const selectedTopic = topicPool[randomIndex];
    console.log(`[Bot] Selected: "${selectedTopic.title}" (type: ${selectedTopic.type}, #${randomIndex + 1}/${topicPool.length})`);
